...
public class ServerInfoScreen extends WindowScreen {
    ...
    public ServerInfoScreen(MLPService.Server server, ServerInfo serverInfo) {
        super(GuiThemes.get(), "Server Info: " + server.displayServerAddress());
        this.server = server;
    ...
    public void initWidgets() {
        ...
        if (notes != null) {
            dataTable.add(
                theme.label("Notes: %s".formatted(notes.strip()))
            );
            dataTable.row();
        }
        ...
        if(server.historical != null) {
            ...
            for (MLPService.ServerPlayer player : server.historical) {
                ...
                if(MeteorClient.mc.world == null) {
                    playersTable.add(theme.button("Login & Join")).expandX().widget().action = () -> {
                        new CrackedAccount(player.name).login();
                        ConnectScreen.connect(this.parent, MeteorClient.mc, ServerAddress.parse(serverInfo.address), serverInfo, false, null);
                    };
                }

                if (server.historical.getLast() != player) playersTable.row();
            }
        } else {
